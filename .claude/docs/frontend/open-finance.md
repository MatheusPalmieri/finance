...
| Transações | Sem "Nova transação", "Importar CSV" nem excluir. O lápis abre **"Reclassificar transação"**: um bloco de leitura com data, conta e valor ("vêm do Open Finance") e os campos do usuário (nome, categoria, forma de pagamento, essencial, recorrência, orçamento, observação). **Tipo de gasto** (Essencial/Não essencial, só em saídas) e **Recorrência** (Fixo/Variável) ficam lado a lado como `Switch` (`components/ui/switch.tsx`) com o estado atual escrito na cor do token e um ícone ⓘ com tooltip explicando cada opção (`ToggleField` em `pages/Transactions/index.tsx`); desligar "Fixo" limpa o orçamento. Quando o nome difere do `originalName`, o rodapé do modal mostra **"Restaurar nome"** (preenche o campo com o nome do banco; vale ao salvar). Estado vazio aponta para `/open-finance`. Selos **"Pendente"** e **"Interno"** |
