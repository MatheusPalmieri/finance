...
| Transações | Sem "Nova transação", "Importar CSV" nem excluir. O lápis abre **"Reclassificar transação"**: um bloco de leitura com data, conta, forma de pagamento e valor ("vêm do Open Finance") e os campos do usuário (nome, categoria, essencial, recorrência, orçamento, observação). **Tipo de gasto** (Essencial/Não essencial, só em saídas) e **Recorrência** (Recorrente/Avulso) ficam lado a lado, cada um como um grupo de toggle buttons (`ToggleGroup` em `components/ui/toggle-group.tsx`) com o botão ativo pintado na cor do token e um ícone ⓘ com tooltip explicando cada opção (`ToggleField` em `pages/Transactions/index.tsx`); desligar "Recorrente" limpa o orçamento. Quando o nome difere do `originalName`, o rodapé do modal mostra **"Restaurar nome"** (preenche o campo com o nome do banco; vale ao salvar). À esquerda do lápis, nas saídas, o cofrinho abre **"Converter em orçamento"**: o mesmo `BudgetModal` da página de Orçamentos (`components/forms/BudgetModal.tsx`, via `initialValues`) já preenchido com o nome (`name`, ou `originalName` se vazio), o valor absoluto como valor fixo e o tipo pelo essencial (Essencial → Essencial, senão Variável) — o usuário só confere e cria; a transação não é vinculada automaticamente. Estado vazio aponta para `/open-finance`. Selos **"Pendente"** e **"Interno"** |
