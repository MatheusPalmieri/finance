...
| Transações | Sem "Nova transação", "Importar CSV" nem excluir. O lápis abre **"Reclassificar transação"**: um bloco de leitura com data, conta e valor ("vêm do Open Finance") e os campos do usuário (nome, categoria, forma de pagamento, essencial, recorrência, orçamento, observação). Quando o nome difere do `originalName`, o rodapé do modal mostra **"Restaurar nome"** (preenche o campo com o nome do banco; vale ao salvar). Estado vazio aponta para `/open-finance`. Selos **"Pendente"** e **"Interno"** |
