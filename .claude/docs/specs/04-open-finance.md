...
**Status:** F0 concluída. As carteiras foram removidas antes da F1
...
| F1 | Schema: `source`, `external_id`, `status` e `kind` (gasto/renda/interno) em `transactions`; tabelas `pluggy_items`, `pluggy_accounts` (vínculo com `accounts`) e payload bruto; conta "Nubank Cartão" | a fazer |
| F2 | Motor de sync: busca → payload bruto → projeção em `transactions`. Classificação só nas novas, campos do usuário nunca sobrescritos, idempotente com lock | a fazer |
